...
function Card( { price, title, image, category, data} ) {

    const { count, setCount, toggleProductDetail, setProductToShow, carProducts, setCartProducts } = useContext(ShoppingCartContext)
    ...
    return (
        <div onClick={() => showProduct(data)} className='bg-white cursor-pointer w-56 h-60 rounded-lg'>
            <figure className='relative mb-2 w-full h-4/5'>
                ...
                <button className='absolute top-0 right-0 flex justify-center items-center bg-white/60 w-6 h-6 rounded-full m-2 p-1' onClick={(e) => {e.stopPropagation(); addProductsToCart(data);}}>
